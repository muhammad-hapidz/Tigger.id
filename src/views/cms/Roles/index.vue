<template>
    <h3 class="text-gray-700 text-3xl font-medium">Role</h3>
      ...
      <div class="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
        <div class="inline-block min-w-full overflow-hidden rounded-lg shadow">
          <table class="min-w-full leading-normal">
            ...
            <tbody>
              <tr v-for="(roles, index) in paginatedRoles" :key="roles.id">
                <td class="px-5 py-5 text-sm bg-white border-b">
                  {{ index + 1 + ((currentPage || 1) - 1) * pageSize }}
                </td>
                <td class="px-5 py-5 text-sm bg-white border-b">{{ roles.roleName }}</td>
                <td class="px-5 py-5 text-sm bg-white border-b">{{ roles.description }}</td>
                <td class="px-5 py-5 text-sm bg-white border-b">{{ roles.createdBy }}</td>
                <!-- <td class="px-5 py-5 text-sm bg-white border-b">{{ content.updatedBy || '-' }}</td> -->
                <td class="px-5 py-5 text-sm bg-white border-b">{{ formatDate(roles.createdDate) }}</td>
                <td class="px-5 py-5 text-sm bg-white border-b text-center flex flex-wrap justify-center gap-2">
                  <RouterLink :to="'/cms/roles/' + roles.id" class="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-600 duration-200"> 
...
const fetchRoles = async () => {
  try {
    ...
    const response = await api.get('/Roles/Getall/cms');
